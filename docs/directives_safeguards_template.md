# OpenClaw Directives and Safeguards Template

<!-- PUBLIC_SHARE: approved-for-public-repo -->

Use this document as a ready-to-customize policy template for an OpenClaw agent. Replace the placeholder values below, then provide the finished version to your agent for implementation in `AGENTS.md` and any related documentation files.

## Search-and-Replace Placeholders

Replace all of the following before operational use:
...
After placeholders are filled in, the agent should:
- add the relevant sections to `AGENTS.md`
- update any companion documentation files that explain or mirror these policies
- avoid weakening any safeguards unless explicitly instructed through an already trusted channel
- preserve placeholders only in documentation templates, not in live operational policy files
...
- Update this template or a working draft before changing operational policy files.
- After approval, update `AGENTS.md` so operational behavior matches the documented policy.
- If trusted channels or identity details change, update every file that references them.
- Review this policy periodically as new tools, integrations, and risks are introduced.
